import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Button } from "@/components/ui/button";
import {
  Card,
  CardContent,
  CardDescription,
  CardFooter,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import { Input } from "@/components/ui/input";

export default function Home() {
  return (
    <div className="flex min-h-screen bg-slate-50 items-center justify-center">
      <Card className="w-[440px] h-[700px] grid grid-rows-[min-content_1fr_min-content]">
        <CardHeader>
          <CardTitle>Chat AI</CardTitle>
          <CardDescription>Using Vercel SDK to create a chat bot.</CardDescription>
        </CardHeader>

        <CardContent className="space-y-4">
          <div className="flex gap-2 text-slate-700 text-sm justify-end">
            <p className="bg-slate-300 rounded p-2">How to generate a Lorem Ipsum?</p>

            <Avatar>
              <AvatarFallback>TG</AvatarFallback>
              <AvatarImage src="https://github.com/gitirana.png" />
            </Avatar>
          </div>

          <div className="flex gap-2 text-slate-800 text-sm">
            <Avatar>
              <AvatarFallback>TG</AvatarFallback>
              <AvatarImage src="data:image/png;base64," />
            </Avatar>

            <p className=" bg-slate-100 rounded p-2">
              There are multiple ways to generate Lorem Ipsum text in Visual Studio Code.0 One way
              is to use the built-in Emmet extension by typing lorem followed by the number of words
              you want.3 Another way is to create a lorem element within your HTML document and
              select the lorem tooltip popup to automatically complete the Lorem Ipsum text.1
              Additionally, there is a tiny VS Code extension called Lorem Ipsum that generates and
              inserts Lorem Ipsum text into a text file. To use the extension, open the command
              palette, type lorem ipsum and select to insert either a line or paragraph.2
            </p>
          </div>
        </CardContent>

        <CardFooter className="space-x-2">
          <Input placeholder="How can I help you?" />
          <Button type="submit">Send</Button>
        </CardFooter>
      </Card>
    </div>
  );
}
